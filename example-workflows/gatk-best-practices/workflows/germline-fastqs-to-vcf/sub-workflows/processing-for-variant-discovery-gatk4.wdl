...
task MarkDuplicates {
  ...
  command {
    ...
    ~{gatk_path} --java-options "-Dsamjdk.compression_level=~{compression_level} -Xmx58G" \
    MarkDuplicates \
...
task GatherBamFiles {
  ...
  command {
    ...
    ~{gatk_path} --java-options "-Dsamjdk.compression_level=6 -Xmx~{xmx}G -XX:+UseShenandoahGC" \
    GatherBamFiles \
    --INPUT ~{sep=' --INPUT ' input_bams} \
    --OUTPUT ~{output_bam_basename}.bam \
    --CREATE_INDEX true \
